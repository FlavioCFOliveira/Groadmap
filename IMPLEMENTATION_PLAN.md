...
**Description:**
Implement the `sprint show` subcommand that displays a comprehensive status report of a sprint. This command provides typical sprint stand-up statistics including task counts and percentages by status (pending, in progress, completed), as well as distribution by severity and criticality levels.

**Technical Requirements:**
- Add `show` subcommand to sprint command in `internal/commands/sprint.go`
- Implement database query to aggregate sprint statistics
- Calculate task counts and percentages for:
  - Pending tasks (BACKLOG + SPRINT status)
  - In progress tasks (DOING + TESTING status)
  - Completed tasks (COMPLETED status)
- Calculate severity distribution across 4 ranges (0-2, 3-5, 6-7, 8-9)
- Calculate criticality distribution (low, medium, high, critical)
- Return JSON output matching SPEC/COMMANDS.md schema
- Support `-r, --roadmap` flag for roadmap selection
- Handle errors: sprint not found, roadmap not specified

**Database Queries Needed:**
- Count tasks by status for a given sprint
- Count tasks by severity range for a given sprint
- Get sprint metadata (description, status)
...
**Validation Requirements:**
- [ ] Command `rmp sprint show -r <name> <id>` returns correct JSON
- [ ] Task counts match actual sprint tasks
- [ ] Percentages are calculated correctly (sum to 100%)
- [ ] Severity distribution uses correct ranges
- [ ] Criticality distribution maps severity to correct levels
- [ ] Error handling for non-existent sprint
- [ ] Error handling for missing roadmap flag
- [ ] All existing tests pass
- [ ] New tests added for sprint show functionality

**Affected Files:**
- `internal/commands/sprint.go` (add show subcommand handler)
- `internal/db/queries.go` (add GetSprintStats function)
...
## Phase 2: Schema Alignment (Critical)

This phase addresses critical discrepancies between the SPEC and the actual implementation. The database schema, models, and commands must align with SPEC/DATABASE.md and SPEC/MODELS.md.
...
**Identified Problem:**
The current database schema in `internal/db/schema.go` defines tasks with fields `description`, `action`, `expected_result`, and `completed_at`. However, SPEC/DATABASE.md specifies tasks should have `title`, `functional_requirements`, `technical_requirements`, `acceptance_criteria`, and lifecycle timestamps (`started_at`, `tested_at`, `closed_at`). This mismatch prevents the application from storing task data according to the specification.

**Technical Description of Need:**
Update the database schema to match SPEC/DATABASE.md:
- Replace `description` with `title`
- Replace `action` with `functional_requirements`
- Replace `expected_result` with `technical_requirements`
- Add `acceptance_criteria` field
- Replace single `completed_at` with lifecycle timestamps: `started_at`, `tested_at`, `closed_at`
- Update composite indexes to cover new field query patterns
- Create migration logic for existing databases
- Update all SQL queries referencing old field names

**Validation Requirements:**
- [ ] Schema DDL matches SPEC/DATABASE.md exactly
- [ ] All indexes from SPEC are created
- [ ] Migration script handles existing data gracefully
- [ ] `EXPLAIN QUERY PLAN` shows proper index usage
- [ ] All existing tests pass with new schema
- [ ] Database file permissions remain 0600

**Affected Files:**
- `internal/db/schema.go` (update tasks table DDL)
- `internal/db/queries.go` (update all field references)

**Files to Create:**
- `internal/db/migrations.go` (schema migration logic)
...
**Identified Problem:**
The `Task` struct in `internal/models/task.go` uses field names `Description`, `Action`, `ExpectedResult`, and `CompletedAt`. SPEC/MODELS.md defines the Task struct with `Title`, `FunctionalRequirements`, `TechnicalRequirements`, `AcceptanceCriteria`, and lifecycle timestamps (`StartedAt`, `TestedAt`, `ClosedAt`). This prevents proper data serialization and violates the specification.

**Technical Description of Need:**
Update the Task struct to match SPEC/MODELS.md:
- Rename `Description` to `Title`
- Rename `Action` to `FunctionalRequirements`
- Rename `ExpectedResult` to `TechnicalRequirements`
- Add `AcceptanceCriteria` field
- Replace `CompletedAt` with `StartedAt`, `TestedAt`, `ClosedAt` pointers
- Update field order for memory alignment (strings first, then pointers, then ints)
- Update `TaskUpdate` struct with new field names
- Update all validation logic for new fields
- Update JSON tags to match SPEC output format

**Validation Requirements:**
- [ ] Task struct fields match SPEC/MODELS.md exactly
- [ ] TaskUpdate struct supports all new fields
- [ ] Validation enforces required fields (title, functional_requirements, technical_requirements, acceptance_criteria)
- [ ] JSON serialization produces correct field names
- [ ] Memory alignment optimized (152 bytes total)
- [ ] All model tests pass

**Affected Files:**
- `internal/models/task.go` (update Task and TaskUpdate structs)
...
**Priority:** P0 - Critical
**Dependencies:** TASK-MODELS-001
**Complexity:** High
**Estimated Time:** 3 days
**Specialists:** go-elite-developer

**Identified Problem:**
The task command handlers in `internal/commands/task.go` use old field names (`--description`, `--action`, `--expected-result`) and don't support the new fields (`--title`, `--functional-requirements`, `--technical-requirements`, `--acceptance-criteria`). This prevents users from creating and editing tasks according to the specification.

**Technical Description of Need:**
Update all task command handlers to use new field names:
- Update `taskCreate` to accept new flags: `--title`, `--functional-requirements`, `--technical-requirements`, `--acceptance-criteria`
- Update `taskEdit` to support editing new fields
- Update `taskGet` to display new field names in output
- Map old flags to new structure or deprecate them
- Update help text to reflect new field names
- Ensure backward compatibility if needed (optional)
- Update all tests to use new field names

**Validation Requirements:**
- [ ] `rmp task create --title "X" --functional-requirements "Y" --technical-requirements "Z" --acceptance-criteria "W"` works
- [ ] `rmp task edit` supports all new fields
- [ ] `rmp task get` returns tasks with new field names
- [ ] Help text shows correct flags
- [ ] All command tests pass
- [ ] Error messages reference correct field names

**Affected Files:**
- `internal/commands/task.go` (update all task command handlers)
- `internal/commands/task_test.go` (update tests)
...
**Identified Problem:**
Per SPEC/STATE_MACHINE.md, status transitions should automatically track lifecycle dates: `started_at` when transitioning to DOING, `tested_at` when transitioning to TESTING, `closed_at` when transitioning to COMPLETED, and clearing all dates when reopening to BACKLOG. The current implementation only tracks `completed_at`.

**Technical Description of Need:**
Implement proper date tracking in state transitions:
- Update `UpdateTaskStatus` in `internal/db/queries.go` to set appropriate timestamp based on transition
...
- Update `taskSetStatus` command handler to capture timestamp once
- Ensure all timestamps use same value within a transaction
- Update tests to verify date tracking behavior

**Validation Requirements:**
- [ ] Transition to DOING sets `started_at` timestamp
- [ ] Transition to TESTING sets `tested_at` timestamp
- [ ] Transition to COMPLETED sets `closed_at` timestamp
- [ ] Reopening to BACKLOG clears all tracking dates
- [ ] Task and audit entries have consistent timestamps
- [ ] State machine tests verify date tracking

**Affected Files:**
- `internal/db/queries.go` (update UpdateTaskStatus)
- `internal/commands/task.go` (update taskSetStatus)
...
**Identified Problem:**
SPEC/COMMANDS.md defines a `rmp task next [num]` command that returns the next N open tasks from the currently open sprint, ordered by severity (descending) then priority (descending). This command is not implemented, preventing users from quickly identifying the most critical tasks for their current sprint.

**Technical Description of Need:**
Implement the `task next` subcommand:
- Add `next` case to `HandleTask` switch in `internal/commands/task.go`
- Query for currently open sprint (status = OPEN)
- Return error if no sprint is open
- Query tasks with status SPRINT, DOING, or TESTING
- Order by severity DESC, then priority DESC
- Limit results to N (default 1, configurable)
- Return empty array if no open tasks
- Add comprehensive tests

**Validation Requirements:**
- [ ] `rmp task next` returns 1 task by default
- [ ] `rmp task next 5` returns up to 5 tasks
- [ ] Tasks ordered by severity DESC, then priority DESC
- [ ] Only returns tasks with status SPRINT, DOING, or TESTING
- [ ] Returns error if no sprint is open
- [ ] Returns empty array if sprint has no open tasks
- [ ] All tests pass

**Affected Files:**
- `internal/commands/task.go` (add next subcommand handler)
- `internal/db/queries.go` (add GetNextTasks function)
...
**Identified Problem:**
SPEC/COMMANDS.md defines a `rmp sprint show -r <name> <id>` command that displays a comprehensive sprint status report including task statistics, progress percentages, severity distribution, and criticality distribution. This command is not implemented, preventing users from getting a quick overview for sprint stand-ups.

**Technical Description of Need:**
Implement the `sprint show` subcommand:
- Add `show` case to `HandleSprint` switch in `internal/commands/sprint.go`
- Query sprint metadata and all tasks in sprint
- Calculate statistics: total, pending (BACKLOG+SPRINT), in_progress (DOING+TESTING), completed
- Calculate progress percentages
- Calculate severity distribution: 0-2, 3-5, 6-7, 8-9
- Calculate criticality distribution: low (0-2), medium (3-5), high (6-7), critical (8-9)
- Return JSON matching SPEC/COMMANDS.md schema exactly
- Handle errors: sprint not found, roadmap not specified
- Add comprehensive tests

**Validation Requirements:**
- [ ] `rmp sprint show -r roadmap 1` returns correct JSON structure
- [ ] Summary counts are accurate
- [ ] Progress percentages sum to 100%
- [ ] Severity distribution uses correct ranges
- [ ] Criticality distribution maps severity correctly
- [ ] Error handling for non-existent sprint
- [ ] Error handling for missing roadmap flag
- [ ] All tests pass
...
| Task | Priority | Complexity | Time | Impact |
|------|----------|------------|------|--------|
| TASK-SCHEMA-001 | P0 | High | 3 days | Schema compliance |
| TASK-MODELS-001 | P0 | High | 2 days | Model compliance |
| TASK-COMMANDS-001 | P0 | High | 3 days | Command compliance |
| TASK-STATE-001 | P1 | Medium | 2 days | State machine |
| TASK-NEXT-001 | P1 | Medium | 2 days | New feature |
| TASK-SPRINT-SHOW-001 | P1 | Medium | 2 days | New feature |
...
- [x] Phase 2: Schema Alignment
  - [x] TASK-SCHEMA-001: Align Database Schema with SPEC
  - [x] TASK-MODELS-001: Update Task Struct to Match SPEC
  - [x] TASK-COMMANDS-001: Update Task Command Handlers
  - [x] TASK-STATE-001: Implement State Machine Date Tracking
  - [x] TASK-NEXT-001: Implement Task Next Command
  - [x] TASK-SPRINT-SHOW-001: Implement Sprint Show Command

---

*Document updated after comprehensive SPEC vs Implementation analysis*
