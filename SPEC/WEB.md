...
- [Routes and Pages](#routes-and-pages)
  ...
  - [Audit History Paths](#audit-history-paths)
  - [Audit History Tree](#audit-history-tree)
...
- **Columns.** The table shows the `AuditEntry` fields defined for the audit entry
  ...
  (the ISO 8601 UTC timestamp), plus the **`Path`** the entry belongs to (see
  [Audit History Paths](#audit-history-paths)). `Path` is derived, not stored: it is
  computed by the server from the entry and the roadmap's current sprint
  membership, and it is what ties an operation to the work it belongs to. The page
  does not redefine the stored fields; `MODELS.md` and `DATABASE.md` remain
  canonical.
- **History paths.** The entries are laid out as branching paths, the way a git
  history reads, so the log states what each operation belongs to rather than
  listing operations side by side. The model is specified in
  [Audit History Paths](#audit-history-paths) and drawn as a tree, specified in
  [Audit History Tree](#audit-history-tree).
...
The audit log records **what** happened but not **what it belonged to**: every
entry names an operation, an entity type (`TASK` or `SPRINT`), that entity's id,
and a timestamp, and nothing else (`MODELS.md § Audit Entry`). Read as a flat
table, six task operations and the sprint close that followed them are seven
unrelated rows.

The data nonetheless supports a real structure. Sprints have a lifecycle in the
log — `SPRINT_CREATE`, `SPRINT_START`, `SPRINT_CLOSE`, `SPRINT_REOPEN` — and the
database knows which sprint each task belongs to. The page therefore assigns every
entry to a **path**, and the paths branch and merge the way a git history does.

1. **The paths.** There are three kinds, and every entry belongs to exactly one:

   | Path | Key | Carries |
   |---|---|---|
   | Roadmap | `roadmap` | The main line. Paths branch from it and merge back into it; it carries no entry of its own. |
   | Sprint | `sprint/<id>` | Every entry whose entity is that sprint, and every entry whose entity is a task that belongs to that sprint. |
   | Backlog | `backlog` | Every entry whose entity is a task that belongs to no sprint. |

2. **Assigning an entry.** An entry with `entity_type` `SPRINT` belongs to
   `sprint/<entity_id>`. An entry with `entity_type` `TASK` belongs to the path of
   the sprint that task is a member of, and to `backlog` when it is a member of
   none. Sprint membership is read through the existing membership lookup used
   elsewhere in the interface (`DATABASE.md § Sprint Tasks`); the page adds no
   query of its own and no new table.

3. **Where a path opens and closes.** A `SPRINT_CREATE` entry **opens**
   `sprint/<id>`, branching it from `roadmap`. A `SPRINT_CLOSE` entry **merges** it
   back into `roadmap`. A `SPRINT_REOPEN` entry opens it again, branching from
   `roadmap` a second time. These three operations are the only branch and merge
   points; every other entry is an ordinary point on its path.

4. **A path may be open before the page begins.** The page shows one page of
   entries (see **Pagination** above), so an entry may belong to a sprint whose
   `SPRINT_CREATE` falls on another page. Such a path simply exists from its first
   entry on this page; its absence of an opening point is not an error and is not
   reported as one.

5. **Membership is current, not historical — a stated limitation.** A task's path
   is the sprint it belongs to **now**, not the sprint it belonged to when the
   operation was recorded. The audit cannot supply the historical answer: a
   `SPRINT_ADD_TASK` or `SPRINT_MOVE_TASK` entry stores the **sprint's** id, not
   the task's (`DATABASE.md § audit Table`), so which task moved is not recorded
   anywhere. A task later moved between sprints therefore shows its whole history
   on its current sprint's path, and a task removed from a sprint shows its whole
   history on `backlog`. This is a property of the recorded data, not of the
   presentation; the interface MUST NOT present the derived path as though it were
   the historical one, and the page states the limitation to the reader.

6. **Derived, never stored.** The path is computed per request from data already
   read. No schema change, no new column, no new table, and no write: the audit
   table is unchanged and the page stays read-only (see
   [Security and Read-Only Guarantees](#security-and-read-only-guarantees)).

### Audit History Tree

The audit page draws the paths of
[Audit History Paths](#audit-history-paths) as a **history tree**: one lane per
path, one point per audit entry, with the branch and merge points that open and
close a sprint's lane. It is the page's presentation of its history; the entry
table below it is the same page of entries read in detail.

1. **Drawn with the vendored library.** The tree is drawn by the vendored
...
5. **The table is not replaced.** The entry table remains, below the tree, with
   ...
   reading of the data; it removes none. This is also what keeps the page whole
   when the drawing script does not run: the tree is client-drawn, so a page
   served without working JavaScript MUST still present every entry in the table,
   with its path, and MUST NOT leave an empty region where the tree would be.

6. **Read-only, and no new route.** The tree introduces no form, no control that
   writes, and no endpoint: it is drawn from the model the server already rendered
   into the page. The drawing script loads from `/static/...` like every other
   client script, so no page carries an inline script and the
   Content-Security-Policy of [Security Headers](#security-headers) is unchanged.
   Every value the script writes into the DOM is written as **text**, never as
   markup, exactly as specified for the task detail modal (see
   [Task Detail Modal](#task-detail-modal)).

7. **The limitation is stated to the reader.** The page states, where the tree is
   shown, that a task's lane is the sprint it belongs to now (see
   [Audit History Paths](#audit-history-paths), rule 5). A derived relationship
   presented as a historical one would be a false claim about the record.

8. **Empty log.** When the roadmap's audit log is empty the page renders the
   empty state specified above and draws no tree: no empty container, no lane, and
   no broken canvas.

9. **Narrow viewports.** The tree stays usable on a small viewport without the
   page overflowing horizontally: the drawing area scrolls within its own
   container if the lanes are wider than the viewport, consistent with
   [Responsive and Mobile-First Design](#responsive-and-mobile-first-design).
