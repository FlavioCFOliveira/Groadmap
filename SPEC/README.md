...
| Write counters on `rmp graph execute` and `rmp graph client` (what a statement changed, when the block is present at all, why a zero is omitted, and why the two property counters are published as one figure) | `GRAPH.md § Write Counters: What a Statement Changed`, with the JSON in `DATA_FORMATS.md § Graph Query Counters` |
