...
- **Audit log as a history tree.** The audit page presents its entries the way a git history reads: one lane per path, a point per operation, with a sprint's lane branching from the roadmap line and merging back when it closes. A sprint's lane carries its own operations and those of its tasks; work in no sprint rides a backlog lane. The lane a task's operation sits on is the sprint it belongs to *now* — the audit records which sprint an operation touched, never which task was added or moved — and the page says so. The entry table stays below the tree, with a Path column, so nothing is lost when the drawing does not run.
