...
The endpoint answers each of its four failure classes with HTTP `400 Bad Request` and a JSON body of exactly two string fields, `error` and `kind`:
...
| `invalid_keyword_spacing` | The query is a schema-introspection command — `SHOW INDEXES`, `SHOW INDEX`, `SHOW CONSTRAINTS`, `SHOW CONSTRAINT` — written with anything other than exactly one space between its two keywords. The guard rail rejects it before execution, because the engine recognises the command only in that spelling and would otherwise fail it in the parser with a diagnostic that names the wrong problem | The keyword spacing and the accepted spelling of the command. It never describes the query as not read-only: a schema-introspection command reads the schema and writes nothing whatever its spacing |
...
- **One status, four kinds.** In each case the server can serve the route and refuses the request the caller made, which is what HTTP `400` states; the `kind` field is what distinguishes the four. A budget exhaustion is neither a `503` (the server is not overloaded and delay alleviates nothing, since the same query exhausts the same budget again) nor a `504` (the server is no gateway or proxy, and the engine is in-process).
- **Precedence.** One request can be wrong in more than one way. The order is `invalid_limit`, then `not_read_only`, then `invalid_keyword_spacing`. The `limit` is resolved before the guard rail runs, so a request carrying both an invalid `limit` and a query that is not read-only is answered `invalid_limit`; and a query that is not read-only is answered `not_read_only` even when it also carries a badly spaced `SHOW`, because the objection that the query writes outranks the objection that it is misspelled. Under any of these orders the request is rejected before the query runs and before the graph store is opened, so neither reads nor writes anything.
