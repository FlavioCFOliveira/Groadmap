...
Two further classes are refused before execution on this endpoint: a schema-introspection command, because the response carries nodes and edges and a schema listing is neither, and a query that reads a relationship bound by an incoming or undirected fixed-length pattern. See [Failure responses](#failure-responses).
...
2. **The query is a statement form that admits no `LIMIT` clause.** Appending one to such a statement bounds nothing — it makes the statement fail in the **parser** — so a read form the guard rail accepts, and that `rmp graph query` runs, would be unusable through this endpoint. One form is affected, and it runs exactly as the caller wrote it:
...
A schema-introspection command — `SHOW INDEXES`, `SHOW INDEX`, `SHOW CONSTRAINTS`, `SHOW CONSTRAINT`, tails included — admits no `LIMIT` either, but this rule never reaches it: the endpoint refuses that class outright, before the injection decision is taken (see [Failure responses](#failure-responses)). It is named here because the reason it is absent from the list is a refusal elsewhere and not an oversight.

The form is recognised on the masked normalization of the query and anchored to the start of the statement, so a keyword appearing only inside a string literal, a comment or a backtick-quoted identifier does not affect the decision, and a `CALL` inside a larger query does not make the statement standalone. Suppression changes no operation class: the form is read-only before the rule and after it, and the guard rail still decides alone whether it executes. A suppressed query is not capped by the node limit; what still bounds it is the time budget above.

### Failure responses

The endpoint answers every failed query-bar request with HTTP `400 Bad Request` and a JSON body of exactly two string fields, `error` and `kind`. `kind` is the machine-readable class; `error` is the human-readable reason the page shows in place. `SPEC/WEB.md § Query-Bar Error Handling`, rule 5, is the canonical enumeration of the `kind` values: a value is added or removed there first, and every other statement of the set refers to it rather than repeating it.

The classes, in the order the endpoint decides them:
...
| `schema_introspection` | The query is a schema-introspection command — `SHOW INDEXES`, `SHOW INDEX`, `SHOW CONSTRAINTS`, `SHOW CONSTRAINT`, with or without a `YIELD`, `WHERE` or `RETURN` tail. The guard rail rejects it before execution, at every keyword spacing, because this page draws nodes and edges and a schema listing is neither | A statement that this page draws a graph and cannot show a schema listing, naming `rmp graph query` as the command that reports the schema. It never names the keyword spacing, and never describes the query as not read-only: a schema-introspection command reads the schema and writes nothing |
| `relationship_read_direction` | The query uses a relationship variable in an expression while that variable is bound by a fixed-length incoming (`<-[e]-`) or undirected (`-[e]-`) pattern. The guard rail rejects it before execution, because the answer the engine returns for such a read cannot be trusted | The relationship variable, the direction of the pattern that bound it, and the outgoing rewrite — including the `UNION ALL` of the two outgoing legs that recovers an undirected traversal in one query |
| `execution` | The query was accepted but then failed in the engine — invalid Cypher syntax, for example — or was cancelled for exhausting the 5-second budget, or was cancelled because the client disconnected | The engine's own diagnostic text, so the user reads the same diagnostic the CLI prints for that query. A query cancelled because the client disconnected names the cancellation rather than the budget, so the budget is never blamed for a caller that gave up |

- **One status for all of them.** In each case the server can serve the route and refuses the request the caller made, which is what HTTP `400` states; the `kind` field is what distinguishes them. A budget exhaustion is neither a `503` (the server is not overloaded and delay alleviates nothing, since the same query exhausts the same budget again) nor a `504` (the server is no gateway or proxy, and the engine is in-process).
- **Precedence.** One request can be wrong in more than one way, and the four guard-rail classes are decided in the order the table lists them. The `limit` is resolved before the guard rail runs, so a request carrying both an invalid `limit` and a query that is not read-only is answered `invalid_limit`. A statement that is both an introspection command and not read-only — `SHOW INDEXES YIELD name CREATE INDEX audit_key FOR (n:Audit) ON (n.key)`, for instance — is answered `not_read_only`, because the objection that it carries a DDL clause outranks the objection that it lists the schema; the same statement without its DDL tail is answered `schema_introspection`. `relationship_read_direction` is last, and its place against `schema_introspection` is unreachable in practice, since an introspection command carries no relationship pattern to orient. Under any of these the request is rejected before the query runs and before the graph store is opened, so it neither reads nor writes anything.
- **Keyword spacing decides nothing here, and the divergence from the CLI is deliberate.** `SHOW  INDEXES` written with two spaces, a tab or a line break receives the same status and the same `kind` as the well-spaced `SHOW INDEXES`, because this endpoint refuses the whole family. Correcting the spacing changes nothing here, so a message naming it would prescribe a correction that does not work and would send the caller round a loop; the single fix, for every spelling, is to ask `rmp graph query` instead. This endpoint therefore publishes no `invalid_keyword_spacing` class. The CLI is the other way round: `graph query`, `graph search` and `graph update` **accept** the schema-introspection class, so on those three the spacing genuinely is the whole objection, and a badly spaced statement is rejected there with a message that names the spacing and the accepted spelling (see [DOCS/commands/graph.md](graph.md)). The two surfaces classify identically — the same recognition, the same two spellings told apart — and differ only in the verdict, because a surface that answers the class owes the caller a working spelling while a surface that refuses the class does not.
