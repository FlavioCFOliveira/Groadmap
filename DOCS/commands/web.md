...
Start a browser-based view of the data the CLI manages. `rmp web` runs an HTTP server embedded in the `rmp` binary (Go standard-library `net/http`) that serves server-rendered HTML and embedded static assets, reading the same on-disk data under `~/.roadmaps/` that the CLI reads.

Every page is read-only and the CLI remains the sole write path for roadmaps, tasks, sprints, comments and the audit log. **One endpoint is not read-only**: the knowledge-graph page's query bar submits an editable Cypher statement to `GET /roadmaps/{name}/graph/data`, which executes it as written — a statement that creates, changes or deletes graph data does so, over an unauthenticated `GET`. See [Security](#read-only-pages-one-writable-endpoint-and-security).
...
| `/roadmaps/{name}/graph/data` | The graph's nodes and edges for the visualisation, produced by executing the Cypher statement in the `q` parameter (the full-graph default query when absent) under the endpoint's 5-second query time budget — against a running graph server when one is serving that roadmap, and against the store directly when none is. The statement is executed as written and may write. An invalid limit or a failed statement answers HTTP `400` with an `error`/`kind` JSON body; a roadmap whose derived socket path is longer than the platform allows is answered HTTP `500` before any statement runs | JSON |
...
**The statement is executed as written, and it may write.** The endpoint does not examine the statement and refuses nothing on the ground of what it does, so a `CREATE`, a `SET`, a `DETACH DELETE` or a schema `CREATE INDEX` typed into the query box is executed, committed and checkpointed, exactly as the same statement would be under `rmp graph execute`. The page shows no confirmation and asks for no credential before running one, and **nothing authenticates the request**: the only access control is the address the server is bound to. See [Security](#read-only-pages-one-writable-endpoint-and-security).

A statement that returns no node and no edge — one that matched nothing, one that returned a number, a `SHOW INDEXES` that returned tabular rows, and a `CREATE` that returned no columns at all — is answered HTTP `200` with `{"nodes": [], "edges": []}`. The four are indistinguishable in the response, because none of them failed: each ran, and none produced an element the response shape can carry. A schema listing is read from `rmp graph execute`, which returns the rows.

### A running graph server serves this endpoint too

`rmp graph serve` holds a roadmap's graph open and answers Cypher over a Unix domain socket (see [DOCS/commands/graph.md](graph.md#running-a-graph-server)). This endpoint follows the same resolution rule the CLI follows, and it follows it automatically:

- **Before it opens anything, the endpoint resolves the roadmap's socket**, `~/.roadmaps/{name}/graph.sock`. When a server answers there, the statement is sent to that server, the store is never opened, and no lock is taken. When nothing answers — the path does not exist, or the connection is refused, which is what a socket file a killed server left behind answers — the endpoint opens the store directly under its exclusive lock, exactly as it did before servers existed. Either way the response is the same response.
- **Nothing configures this.** There is no flag, no query parameter and no environment variable. A server started for a roadmap is used for that roadmap's graph page from the next request onward, and stopping it moves the page back to the direct path just as silently.
- **A statement sent to a server is still executed as written**, writes included, and the security consequence below is unchanged by which path carried it. The server authenticates nobody either.
- **A socket that answers but yields no reachable server is answered HTTP `500`**, and the endpoint does not fall back to the store: a socket that answers may belong to a server holding the lock. A connection lost after the statement was sent is answered HTTP `400` with `kind` `execution`, because the failure surfaced once the statement was already running, and the statement's outcome is genuinely unknown — a commit is durable before it is acknowledged.
- **A derived socket path longer than the platform allows is settled before any of that, and it costs this endpoint its direct path.** No process can bind such a path, so no server can ever listen there, and the endpoint does not read the absent socket as evidence that the roadmap is merely not served. It answers HTTP `500`, opens no store and takes no lock, and the response carries no `kind`, because the request never reached a statement. The graph page still renders, and every fetch it makes for that roadmap's data is refused for as long as the derived path is what it is. The bound is the platform's — 107 bytes on Linux and Windows, 103 on macOS, FreeBSD and OpenBSD, counted in bytes rather than characters — and `~/.roadmaps/{name}/graph.sock` crosses it under a deep enough home directory, with no unusual roadmap name. See [DOCS/commands/graph.md](graph.md#the-socket-path-has-a-length-limit).

**A server started on a non-default socket disables this endpoint for that roadmap, and there is no way around it.** `rmp graph serve --socket <path>` is followed by `rmp graph execute` and `rmp graph client`, which publish the same flag. This endpoint publishes none and can receive none: it is an HTTP handler, `rmp web` serves every roadmap at once rather than one, and no request parameter carries a socket path. It therefore resolves the default path, finds nothing there, concludes the roadmap is not served, and takes the direct path — where it meets the lock the server holds for its whole lifetime, waits out the bounded wait, and answers HTTP `500`. That happens on every request for that roadmap's graph, deterministically, for as long as that server runs. The `500` reports an unavailable store and not a server on another path, because nothing in the product knows that a server is running elsewhere. **Start a server without `--socket` whenever the same roadmap is also browsed.**

**The same absence of a flag is why an over-long derived socket path cannot be worked around from this surface.** On the command line, `rmp graph execute --socket <shorter path>` passes the length check, resolves the roadmap as not served, and opens the store as usual. This endpoint publishes no such flag and can receive no path, so the only thing that brings its graph data back is a shorter derived path: a shorter home directory, or a shorter roadmap name.
...
- **The endpoint executes the caller's statement under a deadline of 5 seconds.** The deadline covers the endpoint's execution of the statement: the run against the engine, the walk over the result that run produces, and the commit. A statement that would run for longer is cancelled.
...
2. **The statement is a form that admits no `LIMIT` clause.** Appending one to such a statement bounds nothing — it makes the statement fail in the **parser** — so a statement that `rmp graph execute` runs would be unusable through this endpoint. Two forms are affected, and each runs exactly as the caller wrote it:
...
| `execution` | The statement failed once it was running — invalid Cypher syntax, for example, or a schema statement the engine refuses — or was cancelled for exhausting the 5-second budget, or was cancelled because the client disconnected | The engine's own diagnostic text, so the user reads the same diagnostic the CLI prints for that statement. A statement cancelled because the client disconnected names the cancellation rather than the budget, so the budget is never blamed for a caller that gave up |
...
- **One status for both.** In each case the server can serve the route and refuses the request the caller made, which is what HTTP `400` states; the `kind` field is what distinguishes them. A budget exhaustion is neither a `503` (the server is not overloaded and delay alleviates nothing, since the same statement exhausts the same budget again) nor a `504` (the server is no gateway or proxy, and the engine is in-process).
...
- **The boundary against `500`.** An internal read error — a failure to open the roadmap's graph store, a lock that cannot be taken within the bounded wait, or a derived socket path longer than the platform allows a socket path to be — is answered `500`, as on every other route, and does not carry this body shape. The two are also separated on the console: a `400` here is a `WARN` (your statement failed), a `500` is an `ERROR` (the server failed). See [Console Log](#console-log). What separates the `500` from the `400` of an execution failure is *when* the failure surfaces: a failure that surfaces once the statement is already running — from the run itself, from the commit, or from the walk over its result — is an execution failure, a store corruption a scan discovers mid-statement included.

- **A refused checkpoint is not one of these, and answers `200`.** A statement that wrote succeeds and commits durably; the checkpoint that folds the committed state into a snapshot runs after that, and a checkpoint that fails never fails the request. It is recorded on the console, with the engine's own error beside it, and changes neither the status nor the body. One cause of that failure does not clear: a committed field longer than the snapshot format accepts is refused by every later checkpoint of that graph, so the write-ahead log is never folded again and every open replays more of it, until a statement shortens or removes the field. The console record says so. The commit-time half of the same condition — a field too long for the write-ahead log — is an ordinary `execution` failure here, carrying the engine's own diagnostic; this endpoint publishes no counterpart to the dedicated line the CLI writes for it. See [DOCS/commands/graph.md](graph.md#how-long-a-field-may-be).
...
- **The graph data endpoint is the exception, and it is a write grant.** `GET /roadmaps/{name}/graph/data` executes the Cypher statement its `q` parameter carries, as written. It does not classify the statement and refuses nothing on the ground of what the statement does, so:
  - a `GET` of that endpoint can create, change and delete nodes, relationships, properties and labels, and can create and drop indexes and constraints. `?q=MATCH (n) DETACH DELETE n` empties the roadmap's knowledge graph, commits, and checkpoints;
...
- **A statement that writes nothing still is not free of on-disk effect.** Opening the store runs the engine's recovery, which repairs an interrupted checkpoint. What such a statement never does is checkpoint: the checkpoint is gated on the write-ahead log having grown, so `snapshot/` and `wal` are left exactly as the request found them.
