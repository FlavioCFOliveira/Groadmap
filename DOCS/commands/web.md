...
Start a browser-based view of the data the CLI manages. `rmp web` runs an HTTP server embedded in the `rmp` binary (Go standard-library `net/http`) that serves server-rendered HTML and embedded static assets, reading the same roadmap databases under `~/.roadmaps/` that the CLI reads. A roadmap's knowledge graph is the one thing it does not read from disk: it reaches that through the roadmap's running graph server, over the same Unix domain socket `rmp graph client` uses (see [How this endpoint reaches the graph](#how-this-endpoint-reaches-the-graph)).

Every page is read-only and the CLI remains the sole write path for roadmaps, tasks, sprints, comments and the audit log. **One endpoint is not read-only**: the knowledge-graph page's query bar submits an editable Cypher statement to `GET /roadmaps/{name}/graph/data`, which sends it to the roadmap's graph server as written — a statement that creates, changes or deletes graph data does so, over an unauthenticated `GET`. See [Security](#read-only-pages-one-writable-endpoint-and-security).
...
Unlike every other command except `rmp graph serve`, `rmp web` is long-lived: it keeps serving until interrupted. Sending `SIGINT` (`Ctrl+C`) or `SIGTERM` shuts the server down gracefully and the process exits 0.
...
time=2026-08-20T19:53:04.226Z level=WARN msg="graph query bar request failed" method=GET path=/roadmaps/platform/graph/data roadmap=platform kind=execution status=400 err="query failed to execute: cypher: parse: unexpected \"RETURN\""
```
...
### How this endpoint reaches the graph

`rmp graph serve` holds a roadmap's graph open and answers Cypher over a Unix domain socket, and it is the only process that opens that store (see [DOCS/commands/graph.md](graph.md#running-a-graph-server)). **This endpoint reaches the graph the way `rmp graph client` reaches it, and by no other means.**

- **It is a client, and it is one in this process.** The endpoint uses the same client mechanism the CLI uses — `internal/graphclient`, linked into the `rmp` binary — to connect to `~/.roadmaps/{name}/graph.sock`, complete the protocol handshake, send the statement and read the result back. It opens no graph store, takes no advisory lock, runs no recovery, creates no `graph/` directory, and checkpoints nothing: a statement that wrote committed inside the server, which checkpoints on its own cadence and at its own shutdown. It also **spawns no subprocess for this**. It does not shell out to `rmp graph client`; the browser launch at startup is the only child process `rmp web` ever creates, and the only one it is allowed to create.
- **What it shares with the CLI is the value mapping, not the document.** Node and relationship values cross one shared mapping, so what the engine said is what both surfaces carry. The documents differ and are meant to: `rmp graph client` publishes the engine's columns and rows, while this endpoint publishes a `{nodes, edges}` view for the visualisation and carries no `columns`, no `rows`, no `counters` and no query plan. A statement whose counters or plan you need is one to run from the command line.
- **Nothing configures this.** There is no flag, no query parameter and no environment variable. The socket is the one derived from the roadmap name, and nothing can point it elsewhere. A server started for a roadmap is used for that roadmap's graph page from the next request onward, and stopping it takes the page back to unreachable just as silently: resolution runs once per request and its outcome is never cached.
- **A statement sent to a server is executed as written**, writes included, and the security consequence below is unchanged by that. The server authenticates nobody either.
- **A lost connection is not retried.** A serialisation conflict inside the server is re-sent under the retry policy, like any other client's; a connection lost or unanswered *after* the statement was sent is not, and is reported as what it is — the outcome is genuinely unknown, because a commit is durable before it is acknowledged. Re-running it could apply it twice, and this process has no store to re-run it against in any case.

### Unreachable, unavailable, and the two answers that must not be confused

**No server reachable is HTTP `503`, recorded at `WARN`.** It covers a socket that does not exist, a socket file a killed server left behind, and a socket that answers without yielding a server. In every one of them this web server is working correctly: the roadmap and its database are readable, every other route is served, and the one thing missing is a process the operator starts with `rmp graph serve -r <name>`. Starting it clears the condition with no change to this server and none to the request, which is exactly what `503` states. `500` is refused for it, because `500` asserts that *this* server failed and would send an operator hunting a fault that is not there. No `Retry-After` is sent: nothing in the product knows when an operator will start a server, so a header carrying a guess would be worse than its absence.

**A derived socket path longer than the platform allows is HTTP `500`, recorded at `ERROR`, and it is settled before anything is probed.** No process can bind such a path, so probing it could tell the handler nothing it does not already know — and what it knows is not "no server happens to be listening" but "none can ever listen here". No delay alleviates that and no operator action short of moving or renaming the roadmap changes it, so announcing a service that will come back would be false. The bound is the platform's — 107 bytes on Linux and Windows, 103 on macOS, FreeBSD and OpenBSD, counted in bytes rather than characters — and `~/.roadmaps/{name}/graph.sock` crosses it under a deep enough home directory, with no unusual roadmap name. See [DOCS/commands/graph.md](graph.md#the-socket-path-has-a-length-limit).

**Keeping the two apart is the point.** They look alike from the browser — both answer the opaque `internal server error` body, and neither carries a `kind`, because neither request ever reached a statement — and they are opposite conditions on the console, which is where an operator reads them. One says *start a server*; the other says *this roadmap's graph can never be served from where it sits*. Recording the transient one at `ERROR` would put an `ERROR` on every page load of a roadmap whose server is simply not running, which trains an operator to ignore the level that means something is broken.

**The page says which state it is in, and never calls an unreachable graph an empty one.** The knowledge-graph page renders in every case — a roadmap with no server still has its page, its query bar and its layout dropdown — and it is the data fetch that fails, not the route. On a `503` the page clears whatever it was showing and displays *This graph cannot be reached*, with `rmp graph serve -r <name>` as the remedy. A graph that a server **is** serving and that simply holds nothing displays *This graph is empty* instead, with a `rmp graph client` example for putting the first node in it. The two states are visually distinct because their remedies have nothing in common, and an interface that showed "no data" for both would tell an operator whose server is stopped that their graph had been emptied.

**A server started on a non-default socket is invisible here, and there is no way around it.** `rmp graph serve --socket <path>` is followed by `rmp graph client`, which publishes the same flag. This endpoint publishes none and can receive none: it is an HTTP handler, `rmp web` serves every roadmap at once rather than one, and no request parameter carries a socket path. It therefore resolves the derived path, learns inside the probe deadline that nothing is listening there, and answers `503` — the same answer it gives for a roadmap nobody is serving, because from where it stands that is the same fact. That happens on every request for that roadmap's graph, deterministically, for as long as that server runs, and the console record names the derived socket rather than the one the server actually bound, because nothing in the product knows that a server is running elsewhere. **Start a server without `--socket` whenever the same roadmap is also browsed.**

**The same absence of a flag is why an over-long derived socket path cannot be worked around from this surface.** On the command line the remedy is real and complete: `--socket` naming a shorter path passes the length check on `rmp graph serve` and on `rmp graph client` alike, so the pair meets there and that roadmap works normally. This endpoint publishes no such flag and can receive no path, so the only thing that brings its graph data back is a shorter derived path — a shorter home directory, or a shorter roadmap name.

### Query time budget: 5 seconds

This web server's own connection timeouts bound the connection, not the work a handler does. A client that sends its headers promptly, stays connected and reads the response as soon as it arrives satisfies all of them however long the handler takes. This endpoint is the one route whose work a caller drives, because the caller writes the query, so the work is bounded by an explicit budget:

- **The statement runs under a budget of 5 seconds, and the graph server is the end that enforces it.** That server takes the value as both its default and its maximum statement timeout, so a statement submitted here cannot raise its own, and one that would run for longer is cancelled there. It is the same budget the CLI's statements run under, read from one declaration, so the two surfaces can never report different figures for it.
...
- **The budget is per request**, and cancelling a statement changes nothing on disk: a cancelled statement's transaction rolls back whole in the server, so it writes no data and appends nothing to the write-ahead log. Nothing is checkpointed by this process in either case — this endpoint opens no store and folds no snapshot, and what a cut statement costs the server is described under [DOCS/commands/graph.md](graph.md#durability-checkpoints-and-shutdown). A statement that completes within the budget is served exactly as it was before the budget existed, with nothing truncated and no latency added.
...
2. **The statement is a form that admits no `LIMIT` clause.** Appending one to such a statement bounds nothing — it makes the statement fail in the **parser** — so a statement that `rmp graph client` runs would be unusable through this endpoint. Two forms are affected, and each runs exactly as the caller wrote it:
   - **A statement with no top-level `RETURN`.** A `LIMIT` attaches only to a projection, so a statement that does not end in one admits none. This covers the standalone procedure call — a statement whose first clause is `CALL` and that has no top-level `RETURN` — and it covers every write with no projection: a `CREATE`, a `SET`, a `DETACH DELETE`, a schema `CREATE INDEX`. A `CALL ... YIELD ... RETURN ...` **is** projected and is an ordinary reading query for this rule, so the resolved limit is injected into it exactly as into a `MATCH ... RETURN`, and so is `MATCH ... SET ... RETURN ...`.
   - **A schema-introspection command** — `SHOW INDEXES`, `SHOW INDEX`, `SHOW CONSTRAINTS`, `SHOW CONSTRAINT`, tails included. The engine's `SHOW` parser rejects `ORDER BY`, `SKIP` and `LIMIT` on every one of those forms, so a `RETURN` tail does not make a `LIMIT` injectable and the rule above would not catch it.

Suppressing a statement with no projection costs nothing, which is why the rule can be that wide: the node limit bounds the **result**, and a statement that projects nothing returns no row, so it contributes no node and no edge to the response whatever is appended to it. The only effect an injection could have on one is to make it fail in the parser.

Each form is recognised on the masked normalization of the statement and anchored to the start of it, so a keyword appearing only inside a string literal, a comment or a backtick-quoted identifier does not affect the decision. Suppression refuses nothing: it decides only whether a `LIMIT` is appended. A suppressed statement is not capped by the node limit; what still bounds it is the time budget above.

### Failure responses

The endpoint answers every failed query-bar request with HTTP `400 Bad Request` and a JSON body of exactly two string fields, `error` and `kind`. `kind` is the machine-readable class; `error` is the human-readable reason the page shows in place. `SPEC/WEB.md § Query-Bar Error Handling`, rule 4, is the canonical enumeration of the `kind` values: a value is added or removed there first, and every other statement of the set refers to it rather than repeating it.

There are exactly two, and there is nothing else the endpoint can refuse:
...
Three further values — `not_read_only`, `schema_introspection` and `relationship_read_direction` — were published while the endpoint still classified a statement before running it. It classifies nothing now, so there is no verdict left for them to carry, and the four-deep precedence rule between them went with them.

- **One status for both.** In each case this server can serve the route and refuses the request the caller made, which is what HTTP `400` states; the `kind` field is what distinguishes them. A budget exhaustion is neither a `503` (delay alleviates nothing, since the same statement exhausts the same budget again) nor a `504` (this server is no gateway or proxy). **A write conflict is a `400` too, and the contrast with the `503` above is what settles it**: there, no graph server could be reached at all; here, one was reached, ran the statement, and went on serving every other request while it did. `503` would announce a service that is unavailable, which that server is not, and would invite a `Retry-After` nothing in the product can compute — nothing knows when the contending writer will stop.
- **The order.** The `limit` is resolved before the statement runs, so a request carrying both an invalid `limit` and an unexecutable statement is answered `invalid_limit` and the statement is never executed. That is not a precedence rule so much as a consequence of the order the endpoint does its work in; `execution` is reached only by a request whose `limit` was accepted.
- **Keyword spacing is the engine's verdict, not this endpoint's.** `SHOW INDEXES` written with a single space is routed to the engine's schema parser, runs, and is answered HTTP `200` with an empty graph. `SHOW  INDEXES` written with two spaces, a tab or a line break is not routed there: it reaches the general Cypher grammar and fails, so it is answered `400` with `kind` `execution` and the engine's own diagnostic, which names `SHOW` rather than the spacing. This endpoint publishes no `invalid_keyword_spacing` class and prescribes no spacing correction. The CLI answers the same way, for the same reason (see [DOCS/commands/graph.md](graph.md)).
- **The boundary against the two 5xx.** Neither of them carries this body shape, because neither request ever reached a statement: a roadmap no server is serving is `503`, and a derived socket path no process could bind is `500`. What separates both from the `400` of an execution failure is *when* the failure surfaces — once the statement is already running, from the run itself or from the walk over its result, it is an execution failure. The three are also separated on the console: a `400` is a `WARN` (your statement failed), the `503` is a `WARN` (a dependency is not running), and the `500` is an `ERROR` (this server, or its environment, is at fault). See [Console Log](#console-log) and [Unreachable, unavailable, and the two answers that must not be confused](#unreachable-unavailable-and-the-two-answers-that-must-not-be-confused).

- **A checkpoint is not this endpoint's to fail.** A statement that wrote commits durably inside the graph server before it is acknowledged; the checkpoint that folds the committed state into a snapshot belongs to that server, on its own cadence and at its own shutdown, so nothing about it reaches this request or this console. One cause of a refused checkpoint never clears — a committed field longer than the snapshot format accepts is refused by every later checkpoint of that graph, so the write-ahead log is never folded again and every open replays more of it, until a statement shortens or removes the field — and the place that reports it is the server's own stderr. The commit-time half of the same condition, a field too long for the write-ahead log, does reach the caller: it is an ordinary `execution` failure here, carrying whatever diagnostic crossed the connection. See [DOCS/commands/graph.md](graph.md#how-long-a-field-may-be).
...
- `SPEC/WEB.md` - full behaviour of the running server (routes, data flow, self-contained delivery, mobile-first design, security)
