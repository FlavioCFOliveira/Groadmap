...
- **A statement cancelled by the time budget can cost gigabytes of memory.** Every mutation a statement has applied is retained in an undo log until the rollback finishes, and nothing bounds how many mutations a statement applies before the budget cuts it. Measured: `MATCH (a),(b),(c) CREATE ()` over a 600-node store of 1.3 MB drove a single `rmp graph execute` process to **3.04 GB** of resident memory at the 5-second budget. The figure tracks the budget rather than the size of the graph. A short-lived invocation returns that memory to the operating system by exiting; `rmp graph serve` and `rmp web` have no exit to return it at, and the connection ceiling bounds how many such statements may run at once but not what each of them costs.
- **A server's shutdown is not bounded.** A statement the budget cut while it was writing is inside an undo replay that takes no cancellation, and the store cannot close until that call returns. The longest such hold measured is **34.5 seconds**, and no ceiling has been established. A supervisor that escalates `SIGTERM` to `SIGKILL` after a short grace period may therefore kill the server mid-replay; every acknowledged commit is still durable and the next open replays the log, but the shutdown checkpoint is lost.
- **A `SET` on a relationship created by `MERGE` in the same statement is silently discarded.** `MERGE (a)-[e:R]->(b) SET e.stamp = 'x' RETURN e.stamp` creates the relationship, returns `null`, writes no property, and exits 0. The identical `SET` issued as its own statement writes correctly, and the same shape on a **node** is correct. Write the relationship first and set its properties in a second statement.
- **An undirected or incoming `SET` on a relationship does not write every relationship it matched.** The same pattern binds more relationships for a `RETURN` than it writes for a `SET`: measured on two relationships either side of one node, `MATCH (n)-[r:R]-(m {key:'b'}) RETURN count(r)` reports 2 while `MATCH (n)-[r:R]-(m {key:'b'}) SET r.stamp = 'x'` writes one of them and still reports `{"ok": true}`. Nothing in the output distinguishes a complete write from a partial one. Write through an outgoing pattern, which can be anchored on either endpoint.
