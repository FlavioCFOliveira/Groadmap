...
An explicit `BEGIN` to `COMMIT` sequence has the same **5 seconds in total** that a single statement has, however many statements it carries, because the maximum statement timeout clamps a transaction's whole life. A session that sends nothing is **not** reclaimed: Groadmap leaves the engine's connection timeout at the engine's own default and arms no idle deadline of its own, so a session held open without being used keeps that session, and the connection slot it occupies, until the client disconnects or the server stops. `rmp graph client` sends one statement per invocation and then disconnects, so it never reaches this; a longer-lived Bolt client is what does. This does not reach the unauthenticated version-negotiation handshake, which the engine bounds separately and unconditionally: a peer that connects and never speaks Bolt at all is still dropped.
