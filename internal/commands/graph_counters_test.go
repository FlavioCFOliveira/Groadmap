...
// Tests for the `counters` member `rmp graph execute` publishes beside a
...
// graphExecuteJSON runs one statement through `rmp graph execute` and returns
...
func graphExecuteJSON(t *testing.T, roadmap, query string) map[string]any {
	t.Helper()
...
// graphExecuteRaw runs one statement and returns its stdout verbatim, for the
// assertions whose subject is the BYTES rather than the decoded object.
func graphExecuteRaw(t *testing.T, roadmap, query string) string {
	t.Helper()
...
func TestGraphCounters_AWriteReportsWhatItChanged(t *testing.T) {
	...
	out := graphExecuteJSON(t, roadmap, "CREATE (:Widget {serial:'A-1', batch:7})")
...
func TestGraphCounters_AReadIsUnchangedInEveryByte(t *testing.T) {
	...
	raw := graphExecuteRaw(t, roadmap, "MATCH (w:Widget) RETURN w.serial")
	const want = "{\n  \"columns\": [\n    \"w.serial\"\n  ],\n  \"rows\": [\n    [\n      \"A-1\"\n    ]\n  ]\n}\n"
	if raw != want {
...
func TestGraphCounters_AMergeThatMatchedIsDistinguishableFromOneThatCreated(t *testing.T) {
	...
	created := graphExecuteJSON(t, roadmap, merge)
	...
	matched := graphExecuteJSON(t, roadmap, merge)
	...
	raw := graphExecuteRaw(t, roadmap, merge)
	if raw != "{\n  \"ok\": true\n}\n" {
...
func TestGraphCounters_ADeleteThatMatchedNoRowReportsNothing(t *testing.T) {
	...
	out := graphExecuteJSON(t, roadmap, "MATCH (w:Widget {serial:'NOT-PRESENT'}) DELETE w")
	...
	count := graphExecuteJSON(t, roadmap, "MATCH (w:Widget) RETURN count(w)")
...
func TestGraphCounters_ADetachDeleteReportsTheEdgeItRemovedAndNoPropertyFigure(t *testing.T) {
	...
	for _, seed := range []string{
		...
		if err := runGraphExecute([]string{"-r", roadmap, "--query", seed}); err != nil {
			t.Fatalf("seeding %q: %v", seed, err)
		}
	}

	out := graphExecuteJSON(t, roadmap, "MATCH (w:Widget {serial:'C-3'}) DETACH DELETE w")
	assertCounters(t, out, map[string]int64{
		"nodesDeleted":         1,
...
func TestGraphCounters_ASchemaStatementReportsItsIndex(t *testing.T) {
	...
	added := graphExecuteJSON(t, roadmap, "CREATE INDEX widget_serial FOR (w:Widget) ON (w.serial)")
	assertCounters(t, added, map[string]int64{"indexesAdded": 1},
		"a CREATE INDEX that registered one index")

	dropped := graphExecuteJSON(t, roadmap, "DROP INDEX widget_serial")
...
func TestGraphCounters_APrefixedStatementPublishesNone(t *testing.T) {
	...
	out := graphExecuteJSON(t, roadmap, "EXPLAIN CREATE (:Widget {serial:'NEVER'})")
	...
	// and no counters.
	profiled := graphExecuteJSON(t, roadmap, "PROFILE MATCH (w:Widget) RETURN w.serial")
...
func TestGraphCounters_TheEngineRefusesAProfileOfAWrite(t *testing.T) {
	...
	_, _ = captureStdStreams(t, func() {
		err = runGraphExecute([]string{"-r", roadmap, "--query", "PROFILE CREATE (:Widget {serial:'X'})"})
	})
	if err == nil {
