		...
		Subcommands: []Subcommand{
			{
				...
				ExitCodes: []ExitCodeEntry{
					ec(0, "The matching tasks were written to stdout as a JSON array; an empty array when the filters match none."),
					ec(2,
						condUnknownFlag,
					...
					ec(6,
						...
						"--priority or --severity falls outside 0-9.",
						"--type carries a value that is not a member of TaskType.",
						"--sort carries a value that is not a member of TaskSort.",
						"--created-since or --created-until is neither RFC3339 nor a bare YYYY-MM-DD date.",
					),
				},
				Examples: []Example{
			...
			},
			{
				...
				Prerequisites: []string{
					"When --parent is supplied, the task it names already exists.",
				},
				ExitCodes: []ExitCodeEntry{
					ec(0, "The task was created in BACKLOG status and its id was written to stdout."),
					ec(2,
						"A required flag was omitted, or supplied with the literal empty string.",
					...
					ec(4,
						condRoadmapNotFound,
						"--parent names a task that does not exist.",
					),
					ec(6,
						"--priority or --severity carries a value outside 0-9.",
						"--type carries a value that is not a member of TaskType.",
						"A free-text field is empty once trimmed, exceeds its cap, is not valid UTF-8, or carries a control character.",
					),
				},
				Examples: []Example{
			...
			},
			{
				...
				Idempotent:  true,
				Prerequisites: []string{
					"Every id on the command line names an existing task; the read is fail-fast and returns nothing when one does not.",
				},
				ExitCodes: []ExitCodeEntry{
					...
					ec(4,
						condRoadmapNotFound,
						"At least one id names no task; nothing is returned.",
					),
					ec(6, "An id in the list falls outside 1-2147483647."),
				},
				Examples: []Example{
			...
			},
			{
				...
				Description: "Returns the next N incomplete tasks from the OPEN sprint, in sprint position order. The listing is NOT filtered by dependencies: a task whose declared dependencies are not yet COMPLETED is returned like any other, at its own planned rank, and nothing in the returned object marks it as blocked. The refusal arrives later and from a different command — the transition to COMPLETED is rejected with exit code 6 while a dependency is still open — so ask `task blockers` before starting work on a task this subcommand returns. This subcommand guarantees the order, not the readiness.",
				...
				Idempotent:  true,
				Prerequisites: []string{
					"A sprint of the roadmap is in OPEN status; start one with `rmp sprint start <id>`.",
				},
				ExitCodes: []ExitCodeEntry{
					...
					ec(4,
						condRoadmapNotFound,
						"No sprint of the roadmap is in OPEN status.",
					),
					ec(6, "The num positional argument is not a positive integer."),
				},
				Examples: []Example{
			...
			},
			{
				...
				Prerequisites: []string{
					...
					"Every task named on the command line is in BACKLOG status; a task in any other status is returned to BACKLOG first.",
					"No task named on the command line still has subtasks.",
				},
				ExitCodes: []ExitCodeEntry{
					...
					ec(4,
						condRoadmapNotFound,
						"At least one id names no task; nothing is deleted.",
					),
					ec(6,
						"A named task is not in BACKLOG status; the whole batch is refused.",
						"A named task still has subtasks.",
			...
			},
			{
				...
				Prerequisites: []string{
					"Every task named on the command line exists.",
					"The requested transition is legal from the current status of every task named (SPEC/STATE_MACHINE.md § Task State Machine).",
					"A task moving to COMPLETED has every declared dependency already COMPLETED.",
					"The commit hash the transition requires is already known to the caller; rmp runs no git command and reads no repository.",
				},
				ExitCodes: []ExitCodeEntry{
					...
					ec(4,
						condRoadmapNotFound,
						"At least one id names no task; nothing is changed.",
					),
					ec(6,
						"The target status is SPRINT, which only sprint operations set.",
						"The target status is not a member of TaskStatus.",
						"The transition is not legal from a named task's current status.",
						"The target is DOING and --commit-open is absent, or the target is COMPLETED and --commit-close is absent.",
						"--commit-open was supplied on a target other than DOING, or --commit-close on a target other than COMPLETED.",
						"A commit hash is not 7 to 64 hexadecimal characters.",
						"--summary was supplied on a target other than COMPLETED, or exceeds 4096 characters, or is not valid UTF-8.",
						"The target is COMPLETED and a named task has a declared dependency that is not itself COMPLETED.",
						"An id in the list falls outside 1-2147483647.",
					),
				},
				Examples: []Example{
			...
			},
			{
				...
				ExitCodes: []ExitCodeEntry{
					...
					ec(6,
						"The priority positional argument falls outside 0-9.",
						"An id in the list falls outside 1-2147483647.",
					),
				},
				Examples: []Example{
			...
			},
			{
				...
				ExitCodes: []ExitCodeEntry{
					...
					ec(6,
						"The severity positional argument falls outside 0-9.",
						"An id in the list falls outside 1-2147483647.",
					),
				},
				Examples: []Example{
			...
			},
			{
				...
				Idempotent:  true,
				Prerequisites: []string{
					"The parent task named on the command line exists.",
				},
				ExitCodes: []ExitCodeEntry{
			...
			},
			{
				...
				Idempotent:  true,
				Prerequisites: []string{
					"Both the dependent task and the blocker task exist.",
					"The edge does not close a cycle in the dependency graph the roadmap already holds.",
				},
				ExitCodes: []ExitCodeEntry{
			...
			},
			{
				...
				Idempotent:  false,
				Prerequisites: []string{
					"A dependency edge runs from the dependent task to the blocker task.",
				},
				ExitCodes: []ExitCodeEntry{
...
func taskCommentAddSubcommand() Subcommand {
	return Subcommand{
		...
		ExitCodes: []ExitCodeEntry{
			ec(0, "The comment was stored against the task and its own id was written to stdout."),
			ec(1,
			...
			ec(2,
				...
				"The task id was omitted, is not an integer, or falls outside 1-2147483647.",
			...
			ec(6,
				"--type carries a value that is not a member of TaskCommentType.",
...
func taskCommentListSubcommand() Subcommand {
	return Subcommand{
		...
		ExitCodes: []ExitCodeEntry{
			ec(0, "The task's comments were written to stdout as a JSON array, oldest first; an empty array when it has none of the requested type."),
			ec(2,
				"The task id was omitted, is not an integer, or falls outside 1-2147483647.",
				"A second positional argument was supplied, or an unrecognised flag was supplied.",
			),
			...
			ec(6, "--type carries a value that is not a member of TaskCommentType."),
		},
		Examples: []Example{
...
func taskCommentEditSubcommand() Subcommand {
	return Subcommand{
		...
		Idempotent:  true,
		Prerequisites: []string{
			"A task comment carries the id named on the command line; it is the comment's own id, not the task's.",
		},
		ExitCodes: []ExitCodeEntry{
			...
			ec(4,
				condRoadmapNotFound,
				"No task comment carries the named id.",
			),
			ec(6,
				"--type carries a value that is not a member of TaskCommentType.",
