...
func buildRoadmapCommand() Command {
	return Command{
		Name:          "roadmap",
		Aliases:       []string{"road"},
		Summary:       "Create, list, and remove roadmaps (~/.roadmaps/<name>/project.db).",
		Description:   "Manages roadmaps, each stored in its own home directory ~/.roadmaps/<name>/ with the SQLite database at ~/.roadmaps/<name>/project.db.",
		...
		Subcommands: []Subcommand{
			{
				...
				Output: SuccessOutput{
					...
					Example: `[{"name":"project1","path":"~/.roadmaps/project1/project.db","size":24576}]`,
				},
				SideEffects: SideEffects{Database: "Read-only.", Filesystem: "Read-only.", Network: "None."},
				Idempotent:  true,
				ExitCodes: []ExitCodeEntry{
					ec(0, "Every roadmap under ~/.roadmaps/ was written to stdout as a JSON array; an empty array when the directory holds none."),
				...
				Examples: []Example{
					...
					{Title: "Unrecognised flag", Cmd: "rmp roadmap list --foo", Stderr: "Error: invalid input: unknown flag: --foo", Exit: 2},
				},
			},
			{
				Name:        "create",
				...
				Description: "Creates the roadmap home directory ~/.roadmaps/<name>/ (mode 0700) and the SQLite database ~/.roadmaps/<name>/project.db (mode 0600) inside it.",
				...
				SideEffects: SideEffects{
					...
					Filesystem: "Creates the roadmap home directory ~/.roadmaps/<name>/ (mode 0700) and ~/.roadmaps/<name>/project.db (mode 0600) inside it.",
					Network:    "None.",
				},
				Idempotent: false,
				Prerequisites: []string{
					"No roadmap of the chosen name exists yet; a roadmap is never replaced by a second create.",
				},
				ExitCodes: []ExitCodeEntry{
			...
			},
			{
				...
				Description: "Deletes the entire roadmap home directory ~/.roadmaps/<name>/ recursively, including project.db and its SQLite sidecars.",
				...
				SideEffects: SideEffects{
					...
					Filesystem: "Removes the entire ~/.roadmaps/<name>/ directory recursively, including project.db and its -wal/-shm sidecars.",
					Network:    "None.",
				},
				Idempotent: false,
				Prerequisites: []string{
					"The roadmap named on the command line exists.",
				},
				ExitCodes: []ExitCodeEntry{
					...
					ec(4, "No roadmap of that name exists under ~/.roadmaps/."),
					ec(6,
		...
		Subcommands: []Subcommand{
			{
				...
				Idempotent:  true,
				ExitCodes: []ExitCodeEntry{
					ec(0, "The roadmap's sprint counts, per-status task counts and average velocity were written to stdout as a JSON object."),
					ec(2,
						condUnknownFlag,
		...
		Subcommands: []Subcommand{
			{
				...
				Idempotent:  true,
				ExitCodes: []ExitCodeEntry{
					ec(0, "The matching BACKLOG tasks were written to stdout as a JSON array; an empty array when the filters match none."),
					ec(2,
						condUnknownFlag,
						condFlagWithoutValue,
						condNonIntegerFlagValue,
						condExcessPositional,
					),
					ec(3, condNoRoadmap),
					ec(4, condRoadmapNotFound),
					ec(6,
						"--limit falls outside 1-100.",
						"--type carries a value that is not a member of TaskType.",
						"--sort carries a value that is not a member of TaskSort.",
						"--priority falls outside 0-9.",
					),
				},
				Examples: []Example{
		...
		Subcommands: []Subcommand{
			{
				...
				Idempotent:  true,
				ExitCodes: []ExitCodeEntry{
					ec(0, "The matching audit entries were written to stdout as a JSON array, newest first; an empty array when the filters match none."),
					ec(2,
						condUnknownFlag,
						condFlagWithoutValue,
						condNonIntegerFlagValue,
					...
					ec(4, condRoadmapNotFound),
					ec(6,
						"--limit falls outside 1-500.",
						"--entity-id falls outside 1-2147483647.",
						"--operation carries a value that is not a member of AuditOperation.",
						"--entity-type carries a value that is neither TASK nor SPRINT.",
						"--since or --until is neither RFC3339 nor a bare YYYY-MM-DD date.",
					),
				},
				Examples: []Example{
			...
			},
			{
				...
				ExitCodes: []ExitCodeEntry{
					...
					ec(4, condRoadmapNotFound),
					ec(6,
						"The entity-type positional argument is neither TASK nor SPRINT.",
						"The entity-id positional argument falls outside 1-2147483647.",
					),
				},
				Examples: []Example{
			...
			},
			{
				...
				Idempotent:  true,
				ExitCodes: []ExitCodeEntry{
					ec(0, "The aggregate counts over the requested window were written to stdout as a JSON object; the counts are zero when the window holds no entry."),
					ec(2,
						condUnknownFlag,
						condFlagWithoutValue,
						condExcessPositional,
					...
					ec(3, condNoRoadmap),
					ec(4, condRoadmapNotFound),
					ec(6, "--since or --until is neither RFC3339 nor a bare YYYY-MM-DD date."),
				},
				Examples: []Example{
