		...
		Subcommands: []Subcommand{
			{
				...
				Prerequisites: []string{
					"No graph server is already running for the roadmap: the store's exclusive lock admits one server at a time, and a second is refused rather than queued.",
					"The graph itself need not exist; starting the server is what creates it, and it is then served empty.",
				},
				ExitCodes: []ExitCodeEntry{
					ec(0, "The server bound its socket, wrote the socket path to stdout, served until SIGINT or SIGTERM, then drained, checkpointed, released the store lock and removed the socket."),
					ec(1,
						"The resolved socket path is longer than the platform allows, whether derived from the roadmap or supplied by --socket.",
						"The graph store's exclusive lock could not be taken within the bounded wait, which is what refuses a second server for the same roadmap.",
						"A live server already answers on the resolved socket.",
						"The socket could not be bound.",
						"The graph store could not be opened or recovered.",
					),
					ec(2,
						"An unrecognised flag was supplied, or a positional argument was supplied; this subcommand accepts none.",
						"--socket was supplied with an empty value.",
					),
					ec(3, condNoRoadmap),
			...
			},
			{
				...
				Idempotent: false,
				Prerequisites: []string{
					"A graph server is running for the roadmap and listening on the resolved socket; start one with `rmp graph serve -r <name>` and leave it running. With nothing listening this subcommand fails rather than opening the store.",
					"A server started on a non-default --socket is reached only by a client passing the same --socket.",
				},
				ExitCodes: []ExitCodeEntry{
					ec(0, "The statement was sent to the server, ran there, and its result was written to stdout."),
					ec(1,
						"No server is listening on the resolved socket, or the resolved socket path is longer than the platform allows.",
						"A server could not be reached through a socket that answered, or the connection was lost or went unanswered after the statement was sent.",
						"The statement failed to parse or to execute in the engine, which is where a duplicate or unknown schema object is reported.",
						"The statement exhausted the 5-second statement time budget, or every attempt of the retry policy lost a serialisation conflict; nothing was written.",
						"The statement was to be read from standard input and the read of the stream failed.",
						"A value the server returned could not be mapped onto the published result shape.",
					),
					ec(2,
						"No statement was supplied: --query is absent and standard input is empty, whitespace only, or a terminal; or --query carries an empty or whitespace-only value.",
						"--socket was supplied with an empty value.",
						"A positional argument was supplied, such as a bare Cypher statement written without --query.",
					),
					ec(3, condNoRoadmap),
					ec(4, condRoadmapNotFound),
					ec(6, "The statement is longer than 1048576 bytes, whether it arrived through --query or through standard input."),
				},
				Examples: []Example{
					{
