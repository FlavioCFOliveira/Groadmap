...
// invocation of the statement subcommand that omitted --query, with a terminal
// on standard input, hung for roughly forty minutes before it was killed. It was
// observed on `rmp graph execute`, since withdrawn; the contract it established
// now binds `rmp graph client`, which reads standard input the same way
...
// `rmp graph client < /dev/zero` report "no query supplied" (exit code 2) about a
