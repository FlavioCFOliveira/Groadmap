...
// TestGroupedTaskCommentsReadIssuesOneStatement is the gate for acceptance
// criterion 3. It measures the statements the grouped read sends for a growing
// number of parents, and contrasts them with the per-task loop the web interface
// must not use (SPEC/WEB.md § Task Detail Modal, "One grouped comment query, never
// N+1").
func TestGroupedTaskCommentsReadIssuesOneStatement(t *testing.T) {
	db, counter, cleanup := setupCountingDB(t)
	defer cleanup()

	// The counter is exercised by the seeding itself, which is what proves it
	// counts at all: an instrument that always reads zero would pass the
	// single-statement assertion for the wrong reason.
	...
	for _, parents := range []int{1, 3, 12} {
		ids := taskIDs[:parents]

		counter.reset()
		grouped, err := db.ListTaskCommentsByTasks(testContext(), ids)
		if err != nil {
			t.Fatalf("ListTaskCommentsByTasks(%d parents): %v", parents, err)
		}
		if got := counter.count(); got != 1 {
			t.Errorf("the grouped read of %d parents issued %d statements, want exactly 1", parents, got)
		}
		if len(grouped) != parents {
			t.Errorf("the grouped read of %d parents returned %d keys, want %d", parents, len(grouped), parents)
		}
		for _, id := range ids {
			if len(grouped[id]) != 2 {
				t.Errorf("grouped[%d] holds %d comments, want 2", id, len(grouped[id]))
			}
		}

		// The alternative the SPEC forbids, measured on the same instrument: one
		// statement per task. This is what makes the assertion above falsifiable.
		counter.reset()
		for _, id := range ids {
		...
		if got := counter.count(); got != parents {
			t.Errorf("the per-task loop over %d parents issued %d statements, want %d; "+
				"the instrument does not track statements one-for-one", parents, got, parents)
		}
	}

	// An empty id set issues no statement at all.
	counter.reset()
	grouped, err := db.ListTaskCommentsByTasks(testContext(), nil)
	if err != nil {
	...
	if len(grouped) != 0 {
		t.Errorf("the grouped read of an empty id set returned %d keys, want 0", len(grouped))
	}
}
